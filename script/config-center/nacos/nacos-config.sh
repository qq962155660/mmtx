...
function addConfig() {
  curl -X POST -H ${1} "http://$2/nacos/v1/cs/configs?dataId=$3&group=MMTX_GROUP&content=$4" >${tempLog} 2>/dev/null
...
if [[ ${failCount} -eq 0 ]]; then
	echo "\033[32m Init nacos config finished, please start mmtx-server. \033[0m"
else
