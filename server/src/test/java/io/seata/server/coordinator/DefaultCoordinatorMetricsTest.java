 ...
 */
package io.mmtx.server.coordinator;
...
import io.mmtx.metrics.Measurement;
    ...
    public void test() throws IOException, TransactionException, InterruptedException {
        ...
        try {
            ...
            Assertions.assertEquals(1,
                    measurements.get("mmtx.transaction(meter=counter,role=tc,status=active)").getValue(), 0);
            ...
            Assertions.assertEquals(0,
                    measurements.get("mmtx.transaction(meter=counter,role=tc,status=active)").getValue(), 0);
            Assertions
                    .assertEquals(1, measurements.get("mmtx.transaction(meter=counter,role=tc,status=committed)").getValue(),
                            0);
            Assertions.assertEquals(1,
                    measurements.get("mmtx.transaction(meter=summary,role=tc,statistic=count,status=committed)").getValue(),
                    0);
            Assertions.assertEquals(1,
                    measurements.get("mmtx.transaction(meter=summary,role=tc,statistic=total,status=committed)").getValue(),
                    0);
            Assertions.assertEquals(1,
                    measurements.get("mmtx.transaction(meter=timer,role=tc,statistic=count,status=committed)").getValue(), 0);
            ...
            Assertions.assertEquals(1,
                    measurements.get("mmtx.transaction(meter=counter,role=tc,status=rollbacked)").getValue(), 0);
            Assertions.assertEquals(1,
                    measurements.get("mmtx.transaction(meter=summary,role=tc,statistic=count,status=rollbacked)").getValue(),
                    0);
            Assertions.assertEquals(1,
                    measurements.get("mmtx.transaction(meter=summary,role=tc,statistic=total,status=rollbacked)").getValue(),
                    0);
            Assertions.assertEquals(1,
                    measurements.get("mmtx.transaction(meter=timer,role=tc,statistic=count,status=rollbacked)").getValue(), 0);
        } finally {
            coordinator.destroy();
