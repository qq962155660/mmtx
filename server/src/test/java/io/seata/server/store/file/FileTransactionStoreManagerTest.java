 ...
 */
package io.mmtx.server.store.file;
...
import io.mmtx.server.session.SessionManager;
import io.mmtx.server.session.file.FileBasedSessionManager;
import io.mmtx.server.store.StoreConfig;
import io.mmtx.server.store.TransactionStoreManager;
import io.mmtx.server.store.TransactionWriteStore;
...
public class FileTransactionStoreManagerTest {
    ...
    public void testBigDataWrite() throws Exception {
        File mmtxFile = Files.newTemporaryFile();
        FileTransactionStoreManager fileTransactionStoreManager = null;
        try {
            fileTransactionStoreManager = new FileTransactionStoreManager(mmtxFile.getAbsolutePath(), null);
            ...
            Assertions.assertTrue(mmtxFile.delete());
        }
    }
    ...
    public void testFindTimeoutAndSave() throws Exception {
        ...
        try {
            ...
            fileTransactionStoreManager = new FileTransactionStoreManager(
                mmtxFile.getAbsolutePath(), sessionManagerMock);
            Assertions.assertTrue((boolean) findTimeoutAndSaveMethod.invoke(fileTransactionStoreManager));

            sessionManager = new FileBasedSessionManager(mmtxFile.getName(), mmtxFile.getParent());
