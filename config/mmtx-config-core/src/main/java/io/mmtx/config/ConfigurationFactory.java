...
public final class ConfigurationFactory {
    ...
    public static final String ENV_PROPERTY_KEY = "mmtxEnv";

    private static final String SYSTEM_PROPERTY_MMTX_CONFIG_NAME = "mmtx.config.name";

    private static final String ENV_MMTX_CONFIG_NAME = "MMTX_CONFIG_NAME";

    public static final Configuration CURRENT_FILE_INSTANCE;

    static {
        String mmtxConfigName = System.getProperty(SYSTEM_PROPERTY_MMTX_CONFIG_NAME);
        if (null == mmtxConfigName) {
            mmtxConfigName = System.getenv(ENV_MMTX_CONFIG_NAME);
        }
        if (null == mmtxConfigName) {
            mmtxConfigName = REGISTRY_CONF_PREFIX;
        }
        String envValue = System.getProperty(ENV_PROPERTY_KEY);
        if (null == envValue) {
            envValue = System.getenv(ENV_SYSTEM_KEY);
        }
        Configuration configuration = (null == envValue) ? new FileConfiguration(mmtxConfigName + REGISTRY_CONF_SUFFIX,
            false) : new FileConfiguration(mmtxConfigName + "-" + envValue + REGISTRY_CONF_SUFFIX, false);
